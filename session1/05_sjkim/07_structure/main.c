...
typedef struct { //8byte 차지하는 Data type 같은데 1byte 짜리 Data Type이 되는 것
    unsigned char b0 : 1;
    unsigned char b1 : 1;
    unsigned char b2 : 1;
    unsigned char b3 : 1;
...
int main(){
    ...
    printf("adc1.MODE is %4X at 0x%p\n", adc1.MODE, &(adc1.MODE));
